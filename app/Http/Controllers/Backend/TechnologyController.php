...
class TechnologyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $reorderRows = Technology::orderBy('display_order')->orderBy('id')->get();
        ...
        try {
            DB::transaction(function () use ($request) {
                foreach ($request->input('order') as $position => $uuid) {
                    Technology::where('uuid', $uuid)->update(['display_order' => $position + 1]);
                }
            });

            ActivityLog::log(config('constants.ACTIVITY_ACTIONS.update'), config('constants.MODULES.technology'), [
                'description' => 'Reordered technologies',
            ]);

            return response()->json(['success' => true]);
        } catch (\Throwable $e) {
            Log::error('Technology reorder failed: ' . $e->getMessage(), ['exception' => $e]);
