...
class PortfolioCategoryController extends Controller
{
        ...
        try {
            DB::transaction(function () use ($request) {
                foreach ($request->input('order') as $position => $uuid) {
                    PortfolioCategory::where('uuid', $uuid)->update(['display_order' => $position + 1]);
                }
            });

            ActivityLog::log(config('constants.ACTIVITY_ACTIONS.update'), config('constants.MODULES.portfoliocategory'), [
                'description' => 'Reordered portfolio categories',
            ]);
    ...
    public function saveorupdate(Request $request, ?string $uuid = null)
    {
        ...
        try {
            ...
            $this->applyImage($request, $data, 'image', 'portfolio-categories', $category, $uuidForUpload);
