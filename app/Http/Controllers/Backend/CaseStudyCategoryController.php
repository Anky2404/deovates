...
class CaseStudyCategoryController extends Controller
{
    ...
    {
        $rows = CaseStudyCategory::withCount('caseStudies')
            ->latest('id')
            ->paginate($this->pagerecords)
            ->withQueryString();
        $reorderRows = CaseStudyCategory::orderBy('display_order')->orderBy('id')->get();
        ...
        try {
            DB::transaction(function () use ($request) {
                foreach ($request->input('order') as $position => $uuid) {
                    CaseStudyCategory::where('uuid', $uuid)->update(['display_order' => $position + 1]);
                }
            });

            ActivityLog::log(config('constants.ACTIVITY_ACTIONS.update'), config('constants.MODULES.casestudycategory'), [
                'description' => 'Reordered case study categories',
            ]);

            return response()->json(['success' => true]);
        } catch (\Throwable $e) {
            Log::error('CaseStudyCategory reorder failed: ' . $e->getMessage(), ['exception' => $e]);
    ...
    {
        $category = $uuid ? CaseStudyCategory::where('uuid', $uuid)->firstOrFail() : null;
        ...
        try {
            ...
            ActivityLog::log(
                ...
                [
                    ...
                    'description' => ($isNew ? 'Created' : 'Updated') . ' case study category: ' . $category->name,
                ]
            );

            DB::commit();

            return redirect()->route('admin.casestudies.categories.index')->with('success', 'Case study category saved successfully.');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('CaseStudyCategory saveorupdate failed: ' . $e->getMessage(), ['exception' => $e]);
        ...
        try {
            ...
            ActivityLog::log(
                ...
                [
                    ...
                    'description' => 'Toggled status of case study category: ' . $category->name,
                ]
            );
            ...
            Log::error('CaseStudyCategory togglestatus failed: ' . $e->getMessage(), ['exception' => $e]);
        ...
        try {
            ...
            ActivityLog::log(
                ...
                [
                    'subject_type' => CaseStudyCategory::class,
                    'subject_id' => $category->id,
                    'description' => 'Deleted case study category: ' . $category->name,
                ]
            );

            return back()->with('success', 'Case study category deleted successfully.');
        } catch (\Throwable $e) {
            Log::error('CaseStudyCategory destroy failed: ' . $e->getMessage(), ['exception' => $e]);
