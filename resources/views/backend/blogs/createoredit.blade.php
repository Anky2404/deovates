...
{{ old('excerpt', $blog->excerpt ?? '') }}</textarea>
                    ...
                    <div id="selected_tags" class="mt-2">
                        @if (isset($blog))
                            @foreach ($blog->tags as $tag)
                                <span class="badge bg-primary me-1 mb-1 selected-tag" data-id="{{ $tag->id }}">
                                    {{ $tag->name }}
                ...
                <div class="col-md-6">
                    ...
                    <textarea name="meta_keywords" id="meta_input" class="form-control json-auto" rows="3"
                        placeholder="seo, blog, marketing">{{ old('meta', !empty($blog->meta_keywords) ? json_encode($blog->meta_keywords, JSON_UNESCAPED_UNICODE) : '') }}</textarea>
                </div>
